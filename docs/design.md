...
3. Parse Markdown using `markdown-it-py` and collect link and image destinations, plus opt-in static HTML attributes.
...
Locations combine Markdown parser block mappings with tracked inline link positions. They usually identify the one-based line containing a link's opening bracket, including links in multiline paragraphs. Reference links are reported at their use rather than at the definition. HTML attribute links identify their opening tag. Unusual nested block containers can still have approximate line numbers; columns are not reported.

## Known limitations

- External HTTP/HTTPS destinations are skipped. Availability, redirects, authentication, and rate limits are not evaluated.
- Static HTML `href`/`src` and arbitrary element IDs are opt-in. HTML parsing is limited to Markdown HTML tokens and Python `HTMLParser`; `srcset`, CSS URLs, dynamic attributes, and browser error recovery are not implemented.
...
The CLI exposes text, JSON, and GitHub Actions formats. Exit code `1` indicates link issues and `2` indicates configuration, input, or read errors. The `0.2.0` release retains JSON schema version `1`; compatibility-affecting changes should be documented in the changelog and tested before a release.
